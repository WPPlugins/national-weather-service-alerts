    ...
    public function __construct($args = array()) {
        ...
        if (isset($zip) && $zip !== null) {
            if (get_site_transient('nws_alerts_xml_' . $zip . $scope) === false) {
                ...
                // Verify that the response code comes back as 'OK' and that the content type is rss+xml
                if (wp_remote_retrieve_response_code($response) == 200 && $response_headers['content-type'] == 'application/rss+xml') {
